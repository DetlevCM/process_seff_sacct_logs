...
GetAllDays <- function() {
  list.files(path = ".", pattern = "*.sacct.log.bz2", full.names = TRUE, recursive = FALSE)
}



# process efficiency data for a specific job return a vector of data
GetEfficiencies <- function(day, job_id) {
  ...
  c(cpu, mem, nbr_nodes, cpu_utilized, cores_per_node)
}
## end GetEfficiencies()
...
ProcessDay <- function(day, unpack_sacct_archive, run_script_in_parallel) {
  ...
  {
    efficiencies <- foreach(i = 1:nrow(jobs)) %dopar% GetEfficiencies(day, jobs$JobID[i])
    ...
    for (i in 1:nrow(jobs)) {
      efficiency <- GetEfficiencies(day, jobs$JobID[i])
